...
func MakeHTTPHandler(ctx context.Context, e Endpoints, logger log.Logger, tracer stdopentracing.Tracer) *mux.Router {
	...
	r.Methods("GET").Path("/news").Handler(httptransport.NewServer(
		ctx,
		...
		decodeListRequest,
		encodeListResponse,
		append(options, httptransport.ServerBefore(opentracing.FromHTTPRequest(tracer, "GET /news", logger)))...,
	))
...
// clients expect that we will encode the slice (array) of news directly,
...
func encodeListResponse(ctx context.Context, w http.ResponseWriter, response interface{}) error {
	...
	return encodeResponse(ctx, w, resp.NewsItems)
}
